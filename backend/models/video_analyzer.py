...
class VideoAnalyzer:
    ...
    def analyze_video(
        ...
        filtered_detection_counts: List[int] = []
        ...
        while True:
            ...
            if frame_idx % frame_interval == 0:
                ...
                if use_two_stage_detection and hasattr(self, 'two_stage_detector'):
                    ...
                    # Collect aggregate stats across frames
                    aggregate_stats = results.setdefault("two_stage_stats", {
                        "validated": 0,
                        "rejected": 0,
                        "validation_time": 0.0,
                    })
                ...
                if detection_metadata:
                    ...
                    results["detection_metadata"].append(detection_metadata)
                    inference_times.append(float(detection_metadata.get("inference_time", 0.0)))
                    raw_detection_counts.append(int(detection_metadata.get("num_raw_detections", len(detections))))
                else:
                    raw_detection_counts.append(len(detections))
                ...
                filtered_detection_counts.append(len(all_detections))
                ...
                results["frames"][frame_key] = {
                    ...
                    "auxiliary": {
                        key: value if isinstance(value, (int, float, str)) else None
                        for key, value in auxiliary_outputs.items()
                        if key not in {"drivable_area_mask", "lane_line_mask"}
                    },
                }
        ...
        avg_raw_detections = float(np.mean(raw_detection_counts)) if raw_detection_counts else 0.0
        avg_filtered_detections = float(np.mean(filtered_detection_counts)) if filtered_detection_counts else 0.0
        total_raw_detections = int(np.sum(raw_detection_counts)) if raw_detection_counts else 0
        results["performance"] = {
            ...
            "avg_raw_detections": avg_raw_detections,
            "avg_filtered_detections": avg_filtered_detections,
