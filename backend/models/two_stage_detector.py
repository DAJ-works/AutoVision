...
class TwoStageDetector:
    ...
    def detect(
        ...
        """Perform two-stage detection and propagate auxiliary outputs."""

        detection_result = self.primary_detector.detect(frame)
        if len(detection_result) == 3:
            detections, primary_metadata, auxiliary = detection_result
        else:
            detections, primary_metadata = detection_result  # type: ignore[misc]
            auxiliary = {}

        metadata: Dict[str, Any] = {
            "primary_detections": len(detections),
            "primary_inference_time": float(primary_metadata.get("inference_time", 0.0)),
            "validated_detections": len(detections),
            "secondary_inference_time": 0.0,
        }
        ...
        for det in detections:
            ...
            # Skip validation when we either trust the detector or the class has no validators.
            if (
                confidence >= max(self.validation_threshold, 0.85)
                or class_name not in self.class_validation_map
                or (class_name == "person" and confidence > 0.6)
            ):
        ...
        metadata["validated_detections"] = len(validated_detections)
        metadata["secondary_inference_time"] = validation_time

        if self.validation_count % 20 == 0:
            logger.debug(
                "Validation stats: %d validated, %d rejected in %.3fs",
                validation_metadata["validated"],
                validation_metadata["rejected"],
                validation_time,
            )
