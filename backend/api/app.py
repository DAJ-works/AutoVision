...
# Disable verbose debug logging now that coordinates are fixed
# logging.getLogger('backend.models.object_detector').setLevel(logging.DEBUG)
...
# Defer detector initialization to avoid blocking server startup
detector = None
tracker = ObjectTracker(max_age=60, min_hits=1, iou_threshold=0.2)  # More forgiving tracker
reidentifier = PersonReidentifier()

def get_detector():
    """Lazy-load the object detector to avoid blocking server startup."""
    global detector
    if detector is None:
        try:
            # Lower confidence threshold for better detection coverage
            # Use hybrid mode: YOLOP for distant vehicles + YOLOv8 for close vehicles
            detector = ObjectDetector(model_size='m', confidence_threshold=0.2, use_hybrid=True)
            logger.info(f"ObjectDetector initialized with confidence_threshold=0.2, hybrid_mode=True")
        except Exception as e:
            logger.error(f"Failed to initialize ObjectDetector: {e}")
            # Return a minimal placeholder that won't crash
            detector = None
    return detector
...
# Alias for AutoVision branding (incidents = cases)
...
# Alias for AutoVision branding
@app.route('/api/incidents/<case_id>', methods=['GET'])
def get_incident(case_id):
    return get_case(case_id)

@app.route('/api/cases/<case_id>/rename', methods=['PUT'])
def rename_case(case_id):
    """Rename a case."""
    data = request.json
    if not data or 'case_name' not in data:
        return jsonify({'error': 'No case name provided'}), 400
    
    new_name = data['case_name'].strip()
    if not new_name:
        return jsonify({'error': 'Case name cannot be empty'}), 400
    ...
    try:
        ...
        old_name = results.get('case_name', '')
        results['case_name'] = new_name
        results['last_modified'] = datetime.now().isoformat()
        
        # Save the updated results
        with open(result_file, 'w') as f:
            json.dump(results, f, indent=2)
        
        logger.info(f"Renamed case {case_id} from '{old_name}' to '{new_name}'")
        ...
        logger.error(f"Error renaming case {case_id}: {e}")
        return jsonify({'error': f'Error renaming case: {str(e)}'}), 500
...
def delete_case(case_id):
    ...
    # Check if it's an active job
    if case_id in active_jobs:
        return jsonify({'error': 'Cannot delete case while analysis is in progress'}), 400
    ...
    try:
        ...
        if case_id in completed_jobs:
            del completed_jobs[case_id]
        
        logger.info(f"Deleted case {case_id} ('{case_name}')")
        ...
        return jsonify({'error': f'Error deleting case: {str(e)}'}), 500
...
# Alias for AutoVision branding (vehicles = persons for backward compatibility)
@app.route('/api/incidents/<case_id>/vehicles', methods=['GET'])
def get_vehicles(case_id):
    return get_persons(case_id)
...
def run_analysis(case_id, case_name, video_path, output_dir, case_details=None):
    ...
    try:
        ...
        if detector is None:
            raise RuntimeError("Object detector failed to initialize - YOLOP model may not be available")
        ...
        try:
            video_analyzer = VideoAnalyzerWithReID(
                ...
                output_dir=str(output_dir),
                enable_reid=True,
                enable_enhanced_filtering=False,  # Disable aggressive filtering for YOLOP
                enable_two_stage_detection=False,  # Disable two-stage validation for YOLOP
        ...
        if person_count == 0 and 'tracks' in results:
            ...
            # Find person tracks - tracks is a list, not a dict
                ...
                for track in tracks:
                    if track.get('class_name', '').lower() == 'person':
                        ...
                        person = {
                            'id': person_id,
                            'metadata': {
                                'appearances': track.get('num_detections', 0),
...
# ============================================================================
# ENHANCED LEGAL ASSISTANT CHAT ENDPOINTS
# ============================================================================

# Using Ollama with LLaMA 3.1 for all chat functionality
# 100% free, runs locally on your machine, no API keys needed!
CHAT_SERVICE_MODE = "ollama"  # Using Ollama with LLaMA 3.1

@app.route('/api/legal-chat', methods=['POST'])
def legal_assistant_chat():
    """
    General legal assistant chat - answers California Vehicle Code questions
    Available anytime, not tied to a specific case
    
    Uses Ollama with LLaMA 3.1 - 100% free, runs locally!
    """
    data = request.json
    ...
    try:
        ...
        # Retrieve relevant CA Vehicle Code rules
        retrieved_rules = analyzer.retrieve_rules(message, top_k=3)
        
        # Build context from rules
        rules_context = "\n\n".join([
            f"**{rule['metadata']['code']}**: {rule['metadata']['title']}\n{rule['document']}"
            for rule in retrieved_rules
        ])
        
        # Build prompt for Ollama
        prompt = f"""You are a California Vehicle Code expert assistant. Answer the following question using the provided CA Vehicle Code rules as reference.
...
=== INSTRUCTIONS ===
- Provide a clear, accurate answer based on the CA Vehicle Code rules above
- Cite specific codes when referencing rules (e.g., "According to CVC 21453(a)...")
- If the question is not related to traffic law, politely redirect to traffic-related topics
- Be professional and helpful

Answer:"""
        
        # Call Ollama
        response_text = analyzer.call_ollama(prompt, max_tokens=500)
        
        return jsonify({"response": response_text})
    
    except Exception as e:
        logger.error(f"Error in legal_assistant_chat: {e}")
        logger.error(traceback.format_exc())
        return jsonify({
            "response": "I apologize, but I encountered an error processing your question. Please make sure Ollama is running (`ollama serve`) and the llama3.1 model is installed (`ollama pull llama3.1`)."
        }), 500

@app.route('/api/chat-enhanced', methods=['POST'])
def enhanced_case_chat():
    """
    Enhanced case-specific chat using CA Vehicle Code expertise
    Combines case analysis data with legal knowledge
    
    Uses Ollama with LLaMA 3.1 and RAG - 100% free, runs locally!
    """
    data = request.json
    ...
    try:
        ...
        person_count = len(case_data.get('person_identities', []))
        total_detections = case_data.get('total_detections', 0)
        
        case_summary = f"""Case: {case_name}
- Persons detected: {person_count}
- Total detections: {total_detections}
- Video: {case_data.get('video_path', 'N/A')}"""
        
        # Retrieve relevant CA Vehicle Code rules based on the question
        retrieved_rules = analyzer.retrieve_rules(message, top_k=3)
        ...
        # Build prompt for Ollama
        prompt = f"""You are analyzing a traffic incident case using California Vehicle Code expertise.
...
=== INSTRUCTIONS ===
- Answer the question based on the case data and CA Vehicle Code rules
- Cite specific codes when applicable (e.g., "According to CVC 21453(a)...")
- Be professional and concise
- If analyzing fault or violations, reference relevant codes
...
Answer:"""
        ...
        logger.error(f"Error in enhanced_case_chat: {e}")
        logger.error(traceback.format_exc())
        return jsonify({
            "response": f"I encountered an error analyzing this case. Please make sure Ollama is running and try again."
        }), 500
